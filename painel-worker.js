...
/* ⚠️ TAMBÉM CAIU JUNTO COM A MESA. Estas regras moravam no meio do bloco da
   cena e sumiram quando ela saiu — o guarda de ossos do build só cobria .proc,
   e a faixa vermelha passou. Lição: quando um bloco grande sai, conferir o
   RENDER, não só o build. Foi um print que mostrou o texto sem caixa.
   Esta é a faixa que avisa que os números não vieram do Organizze agora.
   Ela fica ACIMA dos números, sempre: aviso sobre número vem antes do número. */
.faixa-erro{
...
/* ⚠️ ISTO JÁ FOI DELETADO SEM QUERER UMA VEZ: as regras da procedência
   moravam no meio do bloco da Mesa da Cigana, e sumiram junto quando a Mesa
   saiu. Quem pegou foi o guarda de ossos do build.mjs, não eu. Agora elas
   vivem coladas no aviso do topo, que é o vizinho de verdade delas.

   Aviso que já cumpriu o papel vira nota de rodapé. Este bloco era uma caixa
   amarela com borda e lista de contas — fazia sentido enquanto o caixa estava
   errado. Com o caixa batendo, virou poluição ("achando feio esse layout com
   esses blocos, além de poluírem"). Some não: encolhe. */
...
/* ⚠️ display:grid vence o [hidden] da folha do navegador. Toda classe deste
   arquivo que declara display precisa desta linha logo abaixo. */
...
function recalcular(){
  ...
  /* ⛔ CONTA VENCIDA E NÃO PAGA CONTINUA SENDO COMPROMISSO DO MÊS.
     Antes o filtro era \`p.data >= hoje\`, então uma conta que venceu dia 5 e não
     foi paga simplesmente sumia dos "compromissos do mês" — mas continuava na
     projeção, empurrada pra hoje. Resultado: "sobra" e "teto de hoje" saíam
     otimistas justamente no cenário em que ele mais precisa da verdade.
     Agora: tudo que vence DENTRO do mês e não foi pago conta, vencido ou não. */
  var iniMesLocal = hoje.slice(0,8) + "01";
  var pagarMes = DADOS.aPagar.filter(function(p){ return p.data >= iniMesLocal && p.data <= fimMes; });
  c.aPagarVencidas = pagarMes.filter(function(p){ return p.data < hoje; });
...
function calcularMes(y, c){
  ...
  /* ⛔ \`DADOS.meses\` NUNCA EXISTIU. O worker devolve \`historico.meses\`, que é um
     ARRAY de "YYYY-MM", com as séries \`historico.gastoTotal\` e
     \`historico.rendaLiquida\` no mesmo índice. O código antigo lia um objeto
     inexistente, caía no fallback e mostrava Gasto R$ 0,00 e Renda R$ 0,00 nos
     meses passados — com as categorias logo abaixo mostrando valor de verdade.
     Corrigido em 08/09/2026. */
  var _h = DADOS.historico || {};
  var _i = (_h.meses || []).indexOf(y);
  var g = (_i >= 0) ? { gastoTotal: (_h.gastoTotal || [])[_i], renda: (_h.rendaLiquida || [])[_i] } : null;
  m.gastoCents = (g && typeof g.gastoTotal === "number") ? g.gastoTotal
               : (m.corrente ? DADOS.gastoMes.totalCents : 0);
  m.rendaCents = (g && typeof g.renda === "number") ? g.renda : 0;
  m.temGasto = !!g;
...
function montarLeque(){
  ...
  /* A marca no meio da mão leva pra Hoje. Antes ela abria A Mesa da Cigana,
     a cena de entrada do dia — o Michel pediu pra matar a tela ("qro matar
     essa tela inicial") e ela saiu inteira: markup, CSS e JS. Está no git e
     no artifact "A Mesa da Cigana" se um dia voltar a fazer sentido. */
  sig.setAttribute("aria-label", "Ir para Hoje");
  sig.title = "Hoje";
  sig.innerHTML = sigla();
  sig.addEventListener("click", function(){ irPara("hoje"); });
  mao.appendChild(sig);
