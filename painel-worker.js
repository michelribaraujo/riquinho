...
function conselhoDoDia(c){
  ...
  // 1. Fatura fechando com o mês estourado — é a janela mais cara de perder.
  var maior = null;
  ...
  if (maior && c.mesFecha === false){
    ...
    if (cart){
      ...
      var ate = dias(iso(f1));
      if (ate <= 12) return {
        rot: "A janela", val: "fecha em " + (ate === 0 ? "hoje" : ate + (ate === 1 ? " dia" : " dias")),
        txt: "A fatura do " + cart.nome + " fecha dia " + cart.fechamento + " com " +
             C(Math.abs(maior.saldoCents)) + ". Depois que fechar, parcelar só vale até o vencimento — é a única janela."
      };
    }
  }
