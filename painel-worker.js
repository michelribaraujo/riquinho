...
/* ── O LEQUE ────────────────────────────────────────────────────
   A navegação é uma mão segurando cartas. As inativas estão VIRADAS
   PRA BAIXO — você vê o verso, com o dorso de ouro. A da tela em que
   você está fica VIRADA PRA CIMA: sobe, endireita e mostra a face.

   A geometria vem toda de --pos (-3..-1, 1..3): o ângulo é pos × 7° e
   o pivô fica ABAIXO da carta, então girar já produz o arco do leque
   sozinho — sem precisar calcular translateY pra cada uma.            */
.leque{
...
.leque-nome{
  ...
  color:var(--ouro-txt);opacity:0;transition:opacity .25s ease;
  /* fundo próprio: o rótulo flutua por cima do conteúdo da página e sem
     isso ele se mistura com o texto que está atrás */
...
.leque-mao{
  ...
  /* o topo é folgado de propósito: a carta ativa sobe 16px e cresce 14%,
     e sem esse ar ela vazava por cima do leque e cobria o nome do arcano */
  padding:30px 20px 18px;border-radius:74px;pointer-events:auto;
  /* ⚠️ FUNDO SÓLIDO, não translúcido. Com color-mix + backdrop-filter o texto
     da página atravessava a barra e o leque ficava ilegível — barra de
     navegação não é vidro, é papel por cima do papel. */
  background:var(--sf);
  border:1px solid var(--line2);
  box-shadow:0 14px 38px -16px rgba(20,22,45,.5), 0 2px 0 var(--sf2);
}
.leque-mao::after{
  content:"";position:absolute;inset:4px;border-radius:70px;pointer-events:none;
  border:1px solid var(--ouro);opacity:.20;
}

.carta-nav{
  /* --lift sobe as cartas de fora: girar em torno de um pivô abaixo faz a
     quina de baixo delas descer, e sem esse ajuste elas furam o leque. */
  --pos:0; --lift:0; --rot:calc(var(--pos) * 7deg);
  position:relative;width:46px;height:64px;flex:0 0 auto;margin-inline:-11px;
  border:0;padding:0;background:none;cursor:pointer;
  transform-origin:50% 210%;
  transform:rotate(var(--rot)) translateY(calc(var(--lift) * -1px));
  transition:transform .32s cubic-bezier(.2,.7,.3,1), filter .32s ease;
  z-index:calc(10 - max(var(--pos), calc(var(--pos) * -1)));
}
.carta-nav .dorso,.carta-nav .frente-nav{
  position:absolute;inset:0;border-radius:5px;display:flex;flex-direction:column;
  align-items:center;justify-content:center;gap:2px;
  backface-visibility:hidden;-webkit-backface-visibility:hidden;
  box-shadow:0 3px 10px -5px rgba(20,22,45,.6);
}
/* VERSO — o dorso do baralho: noite com fio de ouro */
.carta-nav .dorso{
  background:var(--noite);border:1px solid var(--ouro);color:var(--ouro-luz);
}
.carta-nav .dorso::after{
  content:"";position:absolute;inset:3px;border:1px solid currentColor;opacity:.3;border-radius:3px;
}
.carta-nav .dorso .rnn{
  font-family:var(--fonte-display);font-size:9px;font-weight:600;letter-spacing:.06em;
  position:absolute;top:4px;left:0;right:0;text-align:center;opacity:.75;
}
/* FACE — só a carta da tela atual fica virada pra cima */
.carta-nav .frente-nav{
  background:var(--sf3,var(--sf));border:1px solid var(--ouro);color:var(--ouro-txt);
  opacity:0;transition:opacity .18s ease;
}
.carta-nav .frente-nav::after{
  content:"";position:absolute;inset:3px;border:1px solid var(--ouro);opacity:.35;border-radius:3px;
}
.carta-nav .frente-nav .rnn{
  font-family:var(--fonte-display);font-size:9px;font-weight:600;
  position:absolute;top:4px;left:0;right:0;text-align:center;
}
.carta-nav svg{width:26px;height:26px}

.carta-nav:hover{transform:rotate(var(--rot)) translateY(calc((var(--lift) + 7) * -1px))}
.carta-nav:focus-visible{outline:2px solid var(--ouro);outline-offset:3px;border-radius:7px}

/* a ativa: sobe, endireita e vira pra cima */
.carta-nav[aria-current="true"]{
  transform:rotate(0deg) translateY(-16px) scale(1.14);
  z-index:20;
}
.carta-nav[aria-current="true"] .dorso{opacity:0}
.carta-nav[aria-current="true"] .frente-nav{opacity:1}

/* o símbolo do riquinho no meio do leque — é a carta "O baralho" */
.leque-sig{
  position:relative;width:44px;height:44px;flex:0 0 auto;margin-inline:6px;
  border:0;padding:0;background:none;cursor:pointer;border-radius:50%;
  transition:transform .3s cubic-bezier(.2,.7,.3,1);z-index:30;
}
.leque-sig svg{width:100%;height:100%;display:block}
.leque-sig:hover{transform:translateY(-6px) scale(1.06)}
.leque-sig:focus-visible{outline:2px solid var(--ouro);outline-offset:3px}
.leque-sig[aria-current="true"]{transform:translateY(-10px) scale(1.12)}
.leque-sig[aria-current="true"]::after{
  content:"";position:absolute;inset:-5px;border-radius:50%;
  border:1px solid var(--ouro);opacity:.65;
}

@media (max-width:560px){
  .carta-nav{width:38px;height:53px;margin-inline:-9px}
  .carta-nav svg{width:21px;height:21px}
  .leque-sig{width:38px;height:38px}
  .leque-mao{padding:24px 14px 10px}
}
@media (prefers-reduced-motion:reduce){
  .carta-nav,.leque-sig{transition:none}
}
...
function montarLeque(){
  ...
  function carta(a, pos){
    ...
    b.innerHTML =
      '<span class="dorso"><span class="rnn">' + a.rn + '</span>' + fig + '</span>' +
      '<span class="frente-nav"><span class="rnn">' + a.rn + '</span>' + fig + '</span>';
