...
.leque-mao{
  ...
  padding:34px 26px 20px;pointer-events:auto;
}
/* A penumbra. Precisa ser generosa: com ela curta demais, o texto da
   página aparecia ENTRE as cartas e o leque virava sopa visual. */
.leque-mao::before{
  content:"";position:absolute;left:-16%;right:-16%;top:-4%;bottom:-34%;z-index:0;
...
.carta-nav{
  --pos:0; --lift:0; --shift:0;
  --rot:calc(var(--pos) * 7deg);
  position:relative;z-index:1;width:48px;height:70px;flex:0 0 auto;margin-inline:-6px;
  border:0;padding:0;background:none;cursor:pointer;
  transform-origin:50% 200%;
  transform:rotate(var(--rot)) translateY(calc(var(--lift) * -1px)) translateX(calc(var(--shift) * 1px));
...
.giro{
  ...
  /* a virada espera a carta subir: primeiro o saque, depois o giro */
  transition:transform .42s cubic-bezier(.3,.05,.25,1) .1s;
}
/* ⚠️ NOME PRÓPRIO, NÃO \`.face\`. Eu reusei \`.face\` aqui e as cartas do
   leque herdaram o \`background:var(--sf)\` das cartas GRANDES do baralho,
   que é declarado depois — resultado: verso e face saíam da mesma cor e a
   carta sacada ficava indistinguível. Mesma lição do renderSelo: antes de
   reaproveitar um nome, \`grep -n "^\\.nome{" index.html\`.              */
.carta-nav .lface{
  position:absolute;inset:0;border-radius:6px;display:flex;align-items:center;justify-content:center;
  backface-visibility:hidden;-webkit-backface-visibility:hidden;
  box-shadow:0 4px 12px -6px rgba(15,17,35,.75);
}
.carta-nav .lface .rnn{
  font-family:var(--fonte-display);font-size:9.5px;font-weight:600;letter-spacing:.06em;
  position:absolute;top:5px;left:0;right:0;text-align:center;
}
.carta-nav .lface::after{
  content:"";position:absolute;inset:3.5px;border:1px solid currentColor;opacity:.3;border-radius:4px;
}
.carta-nav svg{width:27px;height:27px;margin-top:5px}

/* VERSO — o dorso do baralho */
.carta-nav .dorso{background:var(--noite);border:1px solid var(--ouro);color:var(--ouro-luz)}
/* FACE — ⚠️ PERGAMINHO FIXO, não var(--sf). No tema escuro a face saía
   escura igual ao verso e a carta sacada ficava indistinguível das outras:
   só o brilho denunciava. Carta virada pra cima é PAPEL — nos dois temas.
   Estes três valores são o pergaminho, o índigo e o ouro da identidade. */
...
.carta-nav:hover{transform:rotate(var(--rot)) translateY(calc((var(--lift) + 8) * -1px)) translateX(calc(var(--shift) * 1px))}
.carta-nav:focus-visible{outline:2px solid var(--ouro);outline-offset:4px;border-radius:9px}

/* ⭐ A CARTA SACADA — sobe alto o bastante pra NÃO cobrir a vizinha, que
   era o defeito da primeira versão: a ativa crescia e engolia a carta do
   lado. Aqui ela sai da fileira e as vizinhas abrem espaço. */
.carta-nav[aria-current="true"]{
  /* sai do leque PRA FORA (translateX no sentido de --pos), senão ao
     endireitar ela desliza pro centro e senta em cima da vizinha —
     foi assim que o IV sumiu atrás do V. */
  transform:rotate(0deg) translateY(-26px) translateX(calc(var(--pos) * 7px)) scale(1.06);
  z-index:20;
  transition-delay:0s;
}
.carta-nav[aria-current="true"] .giro{transform:rotateY(180deg)}
.carta-nav[aria-current="true"] .frente-nav{
  box-shadow:0 8px 22px -8px rgba(15,17,35,.6), 0 0 0 1px var(--ouro), 0 0 16px -4px var(--ouro);
}

/* o símbolo do riquinho: o meio da mão. Não é carta, não gira. */
.leque-sig{
  position:relative;z-index:15;width:44px;height:44px;flex:0 0 auto;margin:0 8px 4px;
...
.leque-sig:hover{transform:translateY(-7px) scale(1.06)}
.leque-sig:focus-visible{outline:2px solid var(--ouro);outline-offset:4px}
.leque-sig[aria-current="true"]{transform:translateY(-12px) scale(1.14)}
.leque-sig[aria-current="true"]::after{
  content:"";position:absolute;inset:-6px;border-radius:50%;
  border:1px solid var(--ouro);opacity:.7;
}

@media (max-width:560px){
  .carta-nav{width:40px;height:58px;margin-inline:-5px}
  .carta-nav svg{width:22px;height:22px;margin-top:4px}
  .carta-nav .lface .rnn{font-size:8px}
  .leque-sig{width:38px;height:38px;margin:0 5px 3px}
  .leque-mao{padding:30px 14px 16px}
}
...
.baralho{display:grid;grid-template-columns:repeat(auto-fit,minmax(132px,1fr));gap:24px 14px;margin-top:4px}
...
var DADOS = null;
var CALC  = {};
var ESTADO = { tela:"baralho", mes:null, janela:"mes", carregando:false, origem:"—" };
...
function montarLeque(){
  ...
  var esq = ARCANOS.slice(0,3), dir = ARCANOS.slice(3);
  esq.forEach(function(a,i){ mao.appendChild(carta(a, i - 3)); });

  var sig = document.createElement("button");
  sig.type = "button";
  sig.className = "leque-sig";
  sig.dataset.tela = "baralho";
  sig.setAttribute("aria-label", "O baralho");
  sig.title = "O baralho";
  sig.innerHTML = sigla();
  sig.addEventListener("click", function(){ irPara("baralho"); });
  mao.appendChild(sig);

  dir.forEach(function(a,i){ mao.appendChild(carta(a, i + 1)); });
...
function renderBaralho(c){
  var no = el("bBaralho"); if (!no) return;
  el("bEscopo").textContent = "Toque numa carta pra abrir a tela dela.";
  no.innerHTML = "";

  ARCANOS.forEach(function(a){
    var v = versoDoArcano(a.tela, c);
    var slot = document.createElement("div");
    slot.className = "slot";
    slot.innerHTML =
      '<span class="sombra"></span>' +
      '<button class="flip" type="button" aria-label="' + a.nome + ' — abrir ' + TITULOS[a.tela] + '">' +
        '<span class="face frente">' +
          '<span class="rn">' + a.rn + '</span>' +
          '<span class="fig"><svg viewBox="0 0 80 80" width="60" height="60" fill="none" ' +
            'stroke="currentColor" stroke-width="1.2" stroke-linejoin="round">' + a.fig + '</svg></span>' +
          '<span class="nome">' + a.nome + '</span>' +
          '<span class="sig">' + a.sig + '</span>' +
        '</span>' +
        '<span class="face verso">' +
          '<span class="rn">' + a.rn + '</span>' +
          '<span class="vk">' + esc(v.k) + '</span>' +
          '<span class="vv ' + v.tom + '">' + esc(v.v) + '</span>' +
          '<span class="vd">' + esc(v.d) + '</span>' +
          '<span class="vb">' + esc(v.b) + ' ›</span>' +
        '</span>' +
      '</button>';
    no.appendChild(slot);
    slot.querySelector(".flip").addEventListener("click", function(){
      virarCarta(slot, a.tela);
    });
  });
}
...
/* --------------------------------------------------------- navegação ------ */
var TITULOS = { baralho:"O baralho", hoje:"Hoje", pagar:"A pagar", mes:"Fluxo do mês", onde:"Onde vai o dinheiro",
...
try {
  ...
  if (marca){
    ...
    marca.setAttribute("aria-label","Voltar ao baralho");
    marca.addEventListener("click", function(){ irPara("baralho"); });
    marca.addEventListener("keydown", function(ev){
      if (ev.key === "Enter" || ev.key === " ") { ev.preventDefault(); irPara("baralho"); }
    });
  }
